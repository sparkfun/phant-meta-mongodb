...
app.list = function(callback, offset, limit) {
  ...
  Meta.find({hidden: false, flagged: false}).sort({_id: -1}).skip(offset).limit(limit).exec(function(err, streams) {
...
app.listByTag = function(tag, callback, offset, limit) {
  ...
  Meta.find({tags: tag, hidden: false, flagged: false}).sort({_id: -1}).skip(offset).limit(limit).exec(function(err, streams) {
...
app.listByActivity = function(callback, offset, limit) {
  ...
  Meta.find({hidden: false, flagged: false}).sort({last_push: -1}).skip(offset).limit(limit).exec(function(err, streams) {
...
app.flag = function(id, callback) {
  ...
  Meta.findByIdAndUpdate(id, { $set: { flagged: true } }, function(err, stream) {
