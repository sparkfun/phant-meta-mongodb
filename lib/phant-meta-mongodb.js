...
app.listByTag = function(tag, callback, offset, limit) {
  ...
  Meta.find({tags: tag, hidden: { $ne: true }, flagged: { $ne: true }}).sort({_id: -1}).skip(offset).limit(limit).exec(function(err, streams) {
...
app.listByActivity = function(callback, offset, limit) {
  ...
  Meta.find({hidden: { $ne: true }, flagged: { $ne: true }}).sort({last_push: -1}).skip(offset).limit(limit).exec(function(err, streams) {
